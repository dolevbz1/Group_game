...
import CarouselNavShape from './CarouselNavShape';
...
const SECTIONS = [
  ...
  { id: 'news',        title: 'לוח מודעות',        ctaLabel: 'לעדכונים ישנים', navLabel: 'עדכונים',       hasNews: true,      Component: NewsNav },
  { id: 'events',     title: 'אירועים',  ctaLabel: 'לכל האירועים', navLabel: 'אירועים',       hasEvents: true,    Component: EventsNav },
  { id: 'marketplace',title: 'מרקטפלייס', ctaLabel: 'לכל המודעות', navLabel: 'מרקטפלייס',     hasMarketplace: true, Component: MarketplaceNav },
];
...
export default function Carousel({ isReady = false, onEventsOpen, onMarketplaceOpen, onNewsOpen, onStoryOpen, newNewsItems }: CarouselProps) {
  const NEWS_INDEX = SECTIONS.findIndex((s) => s.id === 'news');
  const POLLS_INDEX = SECTIONS.findIndex((s) => s.id === 'polls');
  const VOLUNTEER_INDEX = SECTIONS.findIndex((s) => s.id === 'volunteer');
  ...
  const applyVisuals = useCallback((p: number) => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      icon.style.transform = `scale(${1 + c})`;
      ...
      <div className="carousel-track" ref={trackRef}>
        {SECTIONS.map((section, i) => (
          <article
            ...
            className={`carousel-card carousel-card--${section.id}${i === activeIndex ? ' is-active' : ''}`}
          ...
          >
            ...
            <div className={`carousel-card-body${section.hasVolunteer ? ' is-full-bleed' : ''}${section.hasNews ? ' is-centered' : ''}${section.hasPolls ? ' is-polls' : ''}`}>
              {section.hasNews ? (
                <NewsCard
                  onOpenDetails={onNewsOpen}
                  onStoryOpen={onStoryOpen}
                  onEventsOpen={onEventsOpen}
                  onPollsOpen={() => goTo(POLLS_INDEX)}
                  onVolunteerOpen={() => goTo(VOLUNTEER_INDEX)}
                  onMarketplaceOpen={onMarketplaceOpen}
        ...
        {SECTIONS.map((section, i) => {
          ...
          return (
            <button
              ...
              data-hook={`carousel-nav-${section.id}`}
            ...
            >
              ...
              >
                <CarouselNavShape section={section.id} active={isActive} />
