...
function NewsTicketPreview({ draft }: { draft: ReportNewsDraft }) {
  ...
  useEffect(() => {
    if (contentReady) towerRef.current?.play();
  }, [contentReady]);

  return (
    <div className="report-news-preview" dir="rtl">
      <article className={`news-note${draft.urgent ? ' is-urgent' : ''}`}>
        <div className="news-note-body report-news-ticket-body">
          ...
          <div className={`report-news-ticket-content${contentReady ? ' is-visible' : ''}`}>
            <div className="news-note-visual">
              <Lottie
                lottieRef={towerRef}
                animationData={transmissionTowerAnim}
                loop
                autoplay={false}
            ...
            <div className="news-note-text">
              ...
              <p className="news-note-timestamp text-small-normal">טל לויסנקי · עכשיו</p>
            </div>
          </div>
        </div>
...
type ReportNewsPageProps = {
  open: boolean;
  onClose: () => void;
  onPublished?: (draft: ReportNewsDraft, photoUrl: string | null) => void;
};
